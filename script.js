(() => {
    ...
    $('password-form').addEventListener('submit', async e => { e.preventDefault(); if ($('delete-password').value !== ADMIN_DELETE_PASSWORD || ADMIN_DELETE_PASSWORD === DELETE_PASSWORD_FALLBACK) { $('password-error').classList.remove('hidden'); return; } const record = findItemRecordById(pendingDeleteId); if (!record) return; const found = record.item; undoSnapshot = deepCopy(toolboxData); const cat = record.category; if (cat.subcategories) { const sub = cat.subcategories.find(s => s.name === found.subcategory); if (sub) sub.items = sub.items.filter(item => item.id !== pendingDeleteId); } else cat.items = cat.items.filter(item => item.id !== pendingDeleteId); try { await saveData(); closeModal(passwordModal); pendingDeleteId = null; renderContent(); showToast('工具已刪除，可在 8 秒內復原'); } catch (error) { toolboxData = undoSnapshot; undoSnapshot = null; setStatus('刪除同步失敗，資料已復原', true); } });
