(() => {
    ...
    const CONFIGURED_DELETE_PASSWORD = typeof SUPABASE_CONFIG !== 'undefined' && typeof SUPABASE_CONFIG.deletePassword === 'string' ? SUPABASE_CONFIG.deletePassword : '';
    const DELETE_PASSWORD_CONFIGURED = CONFIGURED_DELETE_PASSWORD.length >= 6 && !CONFIGURED_DELETE_PASSWORD.includes('請自行設定');
    const ADMIN_DELETE_PASSWORD = CONFIGURED_DELETE_PASSWORD || DELETE_PASSWORD_FALLBACK;
    let toolboxData = null, activeCategory = 'ai', activeView = 'category', searchQuery = '', pendingDeleteId = null, undoSnapshot = null, undoTimer = null;
    ...
    async function init() { mainEl.innerHTML = '<div class="empty-state"><p>正在載入資料…</p></div>'; toolboxData = await loadData(); renderViewTabs(); renderTabs(); renderContent(); }
    init();
})();
